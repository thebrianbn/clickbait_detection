...
def tokenise_merged_data(model = "lstm"):
    ...
    instance_data = pd.concat([instance_data1, instance_data2])
    truth_data = pd.concat([truth_data1, truth_data2])
    total_data = pd.merge(instance_data, truth_data)
    ...
    return total_data.sample(frac=1).reset_index(drop = True)
    
def read_subset():
    '''Read only the training data set'''
    
    with open('clickbait17-train-170331/clickbait17-train-170331/instances.jsonl') as f:
        instance_data = pd.DataFrame(json.loads(line) for line in f)
        
    with open('clickbait17-train-170331/clickbait17-train-170331/truth.jsonl') as f:
